...
import {
  FaBullhorn, FaFolder, FaPlay, FaImages, FaBook, FaSearch,
  FaFilePdf, FaFileWord, FaFileExcel, FaChevronLeft, FaChevronRight
} from "react-icons/fa";
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 fixed h-full">
        <div className="p-6">
          <img src="/logo_placeholder.png" alt="스마트팜 코리아" className="h-8" />
        </div>
        <nav className="mt-2">
          {[
            ...
            <a key={idx} href="#" className={`flex items-center px-6 py-3 ${item.active ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-50"}`}>
      ...
      <main className="flex-1 ml-64 p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white p-4 rounded-lg shadow mb-6 flex justify-between">
            <span className="text-sm text-gray-600">총 <strong className="text-blue-600">{notices.length}</strong>건의 검색결과입니다.</span>
            <div className="flex space-x-4">
              <input type="text" placeholder="검색어를 입력해주세요" className="w-64 border border-gray-300 rounded-lg px-4 py-2 text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                ...
                <thead>
                  <tr className="bg-gray-50 text-left">
                    {["번호", "제목", "첨부파일", "작성자", "등록일", "조회"].map((header, idx) => (
                      <th key={idx} className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">{header}</th>
                    ))}
                  </tr>
                </thead>
          ...
          <div className="flex justify-center p-4">
            <button className="px-3 py-2 text-gray-500 hover:bg-gray-100"><FaChevronLeft /></button>
            {[1, 2, 3, 4, 5].map((num) => (
              <button key={num} className="px-4 py-2 text-gray-700 hover:bg-gray-200">{num}</button>
            ))}
            <button className="px-3 py-2 text-gray-500 hover:bg-gray-100"><FaChevronRight /></button>
          </div>
        </div>
      </main>
    </div>
