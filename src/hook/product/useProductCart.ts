...
export const useProductCart = () => {
  ...
  // ✅ 수량 변경 핸들러
  const handleQuantityChange = useCallback((id: number, amount: number) => {
    setCart((prevCart) =>
      prevCart.map((item) =>
  ...
  // ✅ 각 상품의 총 가격 (상품 가격 × 수량)
  const getItemTotal = useCallback((item: CartItemType) => item.price * item.quantity, []);

  // ✅ 개별 상품 기준으로 배송비 적용 (3만 원 이상 무료, 미만 3천 원)
  const getShippingFee = useCallback(
    (item: CartItemType) => (getItemTotal(item) >= 30000 ? 0 : 3000),
    [getItemTotal]
  );
  ...
  const totalPrice = useMemo(
    () => selectedCart.reduce((acc, item) => acc + getItemTotal(item), 0),
    [selectedCart, getItemTotal]
  );

  // ✅ 선택된 상품의 전체 배송비 합산
  const totalShippingFee = useMemo(
    () => selectedCart.reduce((acc, item) => acc + getShippingFee(item), 0),
    [selectedCart, getShippingFee]
  );

  // ✅ 최종 결제 금액
  const finalPrice = useMemo(() => totalPrice + totalShippingFee, [totalPrice, totalShippingFee]);

  // ✅ 네이버페이 결제 요청
  const handlePaymentNaver = useCallback(async (): Promise<void> => {
    if (selectedCart.length === 0) {
      alert("결제할 상품을 선택해주세요.");
      return;
    }
    console.log("선택된 상품:", selectedCart);
    try {
      // ✅ 주문 생성 요청 (배송비 포함)
      const orderResponse = await axios.post(
        `${API_BASE_URL}/order`,
        {
          orderItems: selectedCart.map((item) => ({
            productId: item.productId,
            price: item.price,
            quantity: item.quantity,
          })),
          totalAmount: selectedCart.reduce(
