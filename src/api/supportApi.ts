...
export const getSupportList = async (
    ...
    try {
        const response = await axios.get<SupportListResponse[]>(`${API_BASE_URL}/inquiry/list`, { params: { page, size } });
...
export const insertSupport = async (noticeinsert: { title: string; content: string }): Promise<InsertSupport | null> => {
    const { title, content } = noticeinsert;
    try {
        const response = await axios.post<InsertSupport>(`${API_BASE_URL}/inquiry`, { noticeinsert: { title, content } });
        return response.data;
    } catch (error) {
